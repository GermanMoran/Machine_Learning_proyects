...
# the library sklearn import the descomposition modules
from sklearn.decomposition import PCA
from sklearn.decomposition import IncrementalPCA

# To do a test about this two methods we use a classify called logistic regression
# First we import the logistic regression algorith

# Para escalar los datos entre 0-1
from sklearn.preprocessing import StandardScaler
# Utilizamos una libreria para dividir los datos en conjunto de entrenamiento y testeo
from sklearn.model_selection import train_test_split

# Importamos el modulo de regresion logistica
...
if __name__ == "__main__":
   ...
   # Normalizamos los datos, para PCA siempre es necesario normalizar los  datos, se deben esacalar los datos
   dt_features = StandardScaler().fit_transform(dt_features)

   # Partimos el conjunto de entrenamiento. Para añadir replicabilidad usamos el random state
   # random state para dividir los datos de la misma manera, osea cada vez que se generen los datos no se generen
   # aleatoriamente
   X_train, X_test, y_train, y_test = train_test_split(dt_features, dt_target, test_size=0.3, random_state=42)
   print(X_train.shape)
   print(y_train.shape)
   # LLamamos al algoritmo PCA
   ...
   # Batch size para computadores de bajos recursos donde el entrenamiento se realiza en bloques 
   ipca.fit(X_train)
   '''
